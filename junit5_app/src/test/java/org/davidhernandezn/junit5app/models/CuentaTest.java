...
class CuentaTest {
	...
	void testSaldoCuenta() {
		...
		assertFalse(cuenta.getSaldo().compareTo(BigDecimal.ZERO) < 0);// Verifica que el saldo de la cuenta no sea
																		// negativo.
	...
	void testReferenciaCuenta() {
		...
		// assertNotEquals(cuenta2, cuenta); //VERIFICAR QUE NO SEAN EL MISMO OBJETO EN
		// MEMORIA
		...
		// COMPARAR POR VALOR
		/*
		 * 1- EN CLASE MODEL, VALIDAR OBJETO 2- VALIDAR ATRIBUTOS SI SON NULOS 3-
		 * Compara y RETORNAR por valor de objeto 4- HACER EL TEST DEL
		 */
	...
	/**
	 * @Test void testRelacionBancoCuentas(){ Cuenta cuenta1 = new Cuenta("Jhon",
	 *       new BigDecimal("2500")); Cuenta cuenta2 = new Cuenta("Andres", new
	 *       BigDecimal("1500.8989"));
	 * 
	 *       Banco banco = new Banco(); banco.addCuenta(cuenta1);
	 ...
	 *       assertEquals(2,banco.getCuentas().size()); //Debemos tener la relacion
	 *       inversa al hacer test assertEquals("Banco del Estado",
	 *       cuenta1.getBanco().getNombre());
	 * 
	 *       //Stream QUE PERTENESCA A TAL PERSONA //VALOR ESPERADO
	 *       assertEquals("Andres", banco.getCuentas().stream() .filter(c ->
	 *       c.getPersona().equals("Andres")) .findFirst() //BUSCAR PRIMERO,
	 *       DEVUELVE OPCIONAL .get().getPersona());
	 * 
	 *       /** FORMA 1 assertEquaLs(banco.getCuentas().stream() .filter(c ->
	 *       c.getPersona().equals("Andres")) .findFirst().isPresent(); //BUSCAR
	 *       PRIMERO, DEVUELVE OPCIONAL //FORMA 2
	 *       assertTrue(banco.getCuentas().stream() .anyMatch(c ->
	 *       c.getPersona().equals("Andres"))); }
	 **/

	/**
	 * PARA EL MANEJO DE VARIOS ASSERTION, PODEMOS USAR EXPRESIONES LAMDA
	 * 
	 **/
	...
	void testRelacionBancoCuentas() {
		...
		//AGREGA TU ASSERTS, ESTO PERMITE MOSTRAR EL ERROR DE CUALQUIERA Y NO SE INTERUMPE
		assertAll(() -> {
			assertEquals("1000.8989", cuenta2.getSaldo().toPlainString());
		}, () -> {
			assertEquals("3000", cuenta1.getSaldo().toPlainString());
			...
			assertEquals("Andres", banco.getCuentas().stream().filter(c -> c.getPersona().equals("Andres")).findFirst() // BUSCAR
																														// PRIMERO,
																														// DEVUELVE
																														// OPCIONAL
					.get().getPersona());
